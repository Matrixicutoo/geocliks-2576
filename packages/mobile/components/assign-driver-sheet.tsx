...
/**
 * The delivery-side twin of AssignCrewSheet: who is driving this run.
 *
 * A run carries exactly one driver, so this is a single-select - picking someone replaces
 * whoever held it, and tapping the person who already has it hands the run back to Unassigned
 * (the server drops it to `draft` when it does). Same popup the website shows on the runs list
 * and the run page.
 */
    ...
    <Modal visible transparent animationType="fade" onRequestClose={onClose}>
      <Pressable style={styles.backdrop} onPress={onClose}>
        ...
        >
          <View style={[styles.head, { borderColor: colors.border }]}>
            <View style={{ flex: 1 }}>
              ...
              {routeName ? (
                <Text
                  numberOfLines={1}
                  style={[styles.sub, { color: colors.mutedForeground, fontFamily: Fonts?.mono }]}
                >
          ...
          <Text style={[styles.body, { color: colors.mutedForeground }]}>{t("driver.body")}</Text>
                ...
                return (
                  <View key={member.id} style={[styles.row, { borderColor: colors.border }]}>
                    {member.user?.image ? (
                      ...
                      <View
                        style={[styles.avatar, styles.avatarEmpty, { borderColor: colors.border }]}
                      >
                        <Text
                          style={[styles.initials, { color: colors.amber, fontFamily: Fonts?.mono }]}
                        >
                    ...
                    <Pressable
                      accessibilityLabel={on ? t("driver.assigned") : t("driver.add")}
                    ...
                    >
                      ...
                      >
                        {(on ? t("driver.assigned") : t("driver.add")).toUpperCase()}
                      </Text>
                    </Pressable>
                  </View>
                );
          ...
          <Pressable
            accessibilityLabel={t("driver.done")}
            onPress={onClose}
