...
// `dev:desktop` runs Vite from packages/desktop, so the root .env is not loaded here and
// WEBSITE_URL can be undefined in dev — fall back to the template-managed website port.
const WEB_DEV_URL = process.env.WEBSITE_URL ?? `http://localhost:${ports.website}`;
