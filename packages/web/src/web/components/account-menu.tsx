  ...
  return (
    <div ref={ref} className="relative shrink-0">
      <button
        ...
        aria-expanded={open}
        className={cn(
          "rounded-[6px] flex items-center gap-2 border px-1.5 py-1.5 transition-colors",
          open ? "border-amber text-chalk" : "border-line text-fog hover:border-amber/60",
        )}
      >
        {image ? (
          <img src={image} alt="" className="size-7 rounded-[8px] object-cover" />
        ) : (
          <span className="mono flex size-7 items-center justify-center rounded-[8px] bg-amber text-[11px] font-semibold text-on-amber">
      ...
      {open && (
        <div className="absolute right-0 top-[calc(100%+8px)] z-50 w-[268px] rounded-[12px] border border-line bg-ink-2 shadow-[0_18px_50px_rgba(0,0,0,0.45)]">
          ...
          <div className="p-1.5">
            ...
            >
              <UserCircle className="size-4 text-amber" /> {t("profile.title")}
            </Link>
            <Link
              to="/app/billing"
              onClick={() => setOpen(false)}
            ...
            {me.data?.staffRole && (
              <Link
                to="/admin"
                onClick={() => setOpen(false)}
                className="rounded-[8px] flex items-center gap-2.5 px-2.5 py-2 text-[12.5px] text-alert transition-colors hover:bg-ink-3"
              >
            ...
            <button
              type="button"
              onClick={toggle}
              className="rounded-[8px] flex w-full items-center gap-2.5 px-2.5 py-2 text-[12.5px] text-fog transition-colors hover:bg-ink-3 hover:text-chalk"
            >
              {theme === "dark" ? (
                <Sun className="size-4 text-amber" />
              ) : (
                <Moon className="size-4 text-amber" />
              )}
            ...
            <button
              ...
              className="rounded-[8px] flex w-full items-center gap-2.5 border-t border-line px-2.5 py-2 text-[12.5px] text-fog transition-colors hover:bg-ink-3 hover:text-alert"
            >
