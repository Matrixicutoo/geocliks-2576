...
const ROLES = ["owner", "admin", "manager", "field"] as const;
...
export default function AppTeam() {
  ...
  const projectName = (id_: string) =>
    activeProjects.find((p) => p.id === id_)?.name ?? "Project";
    ...
    >
      <div className="grid gap-6 xl:grid-cols-[minmax(0,1fr)_340px]">
        <div className="min-w-0 space-y-6">
          <div className="rounded-[12px] border border-line bg-ink-2">
            <div className="border-b border-line px-4 py-3">
              <p className="label text-fog">Members</p>
            </div>
            {team.isLoading ? (
                  ...
                  >
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="mono truncate text-[10.5px] text-fog">
                        {member.user?.email} · {member.photoCount} photos · joined{" "}
                    ...
                    <button
                      type="button"
                      aria-label={`Projects for ${member.user?.email ?? "member"}`}
                      onClick={() => setProjectsFor(projectsFor === member.id ? null : member.id)}
                      className={cn(
                    ...
                    {projectsFor === member.id && (
                      <div className="w-full rounded-[12px] border border-line bg-ink p-3">
                        <p className="label mb-2 text-fog">
                          Project access · {member.user?.email}
                        </p>
                        {activeProjects.length === 0 ? (
                          <p className="text-[11.5px] text-fog">No active projects yet.</p>
                        ) : (
                              ...
                              return (
                                <button
                                  ...
                                  className={cn(
                                    ...
                                    on
                                      ? "bg-amber/10 text-chalk"
                                      : "text-fog hover:bg-ink-3/60",
                                  )}
                                >
                                  <span
                                    className={cn(
                        ...
                        {member.role !== "field" && (
                          <p className="mt-2 text-[11.5px] text-fog">
                            {member.role === "owner" ? "Owners" : "This role"} already sees every
                            project — assignments only change what field members can capture into.
                          </p>
                        )}
                      </div>
                    )}
          ...
          <div className="rounded-[12px] border border-line bg-ink-2">
            <div className="border-b border-line px-4 py-3">
              <p className="label text-fog">Pending invites</p>
            </div>
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        {parseInviteProjects(row.projectIds).length > 0 && (
                          <p className="mt-0.5 truncate text-[11px] text-fog">
                            ...
                            {parseInviteProjects(row.projectIds)
                              .map(projectName)
                              .join(", ")}
                          </p>
                        )}
                        ...
                        <button
                          ...
                          onClick={async () => {
                            ...
                            try {
                              ...
                              setNotice(`Invite to ${row.email} revoked. The code no longer works.`);
                            } catch (err) {
                              setError(err instanceof Error ? err.message : String(err));
                            }
                          }}
                      ...
                      <div className="mt-3 rounded-[12px] border border-line bg-ink p-4">
                        {qr.isPending ? (
                          <p className="mono flex items-center gap-2 text-[11px] text-fog">
                            <Loader2 className="size-3.5 animate-spin" /> Building QR…
                          </p>
                        ) : qr.isError ? (
                          ...
                          <div className="flex flex-col items-center gap-3">
                            <img
                              src={qr.data.dataUrl}
                              alt={`Invite QR code for ${row.email}`}
                            ...
                            <div className="flex w-full gap-2">
                              ...
                              >
                                <Copy className="mr-1 inline size-3" /> Copy link
                              </button>
                              <a
                            ...
                            <p className="text-center text-[11px] leading-relaxed text-fog">
                              Scanning this joins {org.data?.name ?? "this workspace"} as{" "}
                              <span className="text-chalk">{row.role}</span>. Print it or hold up
                              your phone — no code to type.
                            </p>
                          </div>
                        )}
                      </div>
        ...
        <div className="space-y-4">
          ...
          <form
            onSubmit={async (event) => {
              ...
              try {
                ...
                setNotice(
                  res.emailSent
                    ? `Invite emailed to ${sentTo} — code ${res.code}. Assign their projects from Teamspace once they join.`
                    : `Invite created — code ${res.code}. No email was sent${
                        res.emailReason ? ` (${res.emailReason})` : ""
                      }. Share the link or QR yourself.`,
                );
              } catch (err) {
                setError(err instanceof Error ? err.message : String(err));
              }
          ...
          >
            <div className="border-b border-line px-4 py-3">
              <p className="font-display text-[15px] font-semibold">Invite a crew member</p>
            </div>
            <div className="space-y-3 p-4">
              <label className="block">
                <span className="label mb-1.5 block text-fog">Work email</span>
                <input
              ...
              <div>
                <span className="label mb-1.5 block text-fog">
                  Role <span className="text-alert">*</span>
                </span>
 ...
 className="rounded-[8px] inline-flex w-full items-center justify-center gap-2 bg-amber px-4 py-2.5 text-[13px] font-semibold text-ink disabled:opacity-60"
          ...
          <div className="rounded-[12px] border border-line bg-ink-2">
            ...
            <div className="space-y-4 p-4">
              <div>
                ...
                <p className="mt-1.5 text-[11.5px] text-fog">
                  {override === null
                    ...
                    : `This browser is set to ${theme}. Nobody else is affected.`}
                </p>
              </div>
              ...
              {isAdmin && (
                <div className="border-t border-line pt-4">
                  <span className="label mb-1.5 block text-fog">{lang.t("appearance.workspaceDefault")}</span>
                  <div className="flex gap-1.5">
                    {(["light", "dark"] as const).map((item) => (
                      ...
                      >
                        {item === "light" ? <Sun className="size-3.5" /> : <Moon className="size-3.5" />}
                        {item}
                      </button>
                    ))}
                  </div>
                  <p className="mt-1.5 text-[11.5px] text-fog">
                    Applies on web, mobile and desktop for everyone who has not picked their own.
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="rounded-[12px] border border-line bg-ink-2 p-4">
            <p className="flex items-center gap-2 text-[13px] font-semibold text-chalk">
              <ShieldCheck className="size-4 text-verified" /> Project-level access
            </p>
            <p className="mt-1.5 text-[12.5px] leading-relaxed text-fog">
              Field users only see projects they are assigned to. Tick projects on the invite form
              to have them assigned the moment the invite is accepted, or use the{" "}
              <span className="text-chalk">folder button</span> on any member above to change
              access after they have joined. Each project page also has{" "}
              <span className="text-chalk">Crew access</span>.
            </p>
          </div>
          )}
